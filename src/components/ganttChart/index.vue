...
<template>
  <div class="gantt-table">
      ...
      <div class="left" ref="left">
        <div class="head">
          ...
          <div class="head-box">
            <GanttRow>
              ...
              <GanttBlock>类型</GanttBlock>
              <GanttBlock class="text-line-1" style="width: 800px;">设备</GanttBlock>
        ...
        <div class="left-box" :style="{ transform: `translateY(-${transFormYValue}px)` }">
          <GanttRow v-for="(item, index) in ganttData" :key="index">
            <GanttBlock :style="_mergeStyle('typeName', item.typeName, index)">{{ item.typeName }}</GanttBlock>
            <GanttBlock class="text-line-1" style="width: 800px;">{{ item.name }}</GanttBlock>
            <GanttBlock>{{ item.model }}</GanttBlock>
      ...
      <div class="right" ref="right">
        ...
        <div class="head">
          <div class="head-box">
            <GanttRow :style="{ transform: `translatex(-${transFormXValue}px)` }" class="transition-transForm">
              <GanttTimeBlock :target-time="_formatDate(item)" v-for="(item, index) in choiceTimeArr" :key="index">
              </GanttTimeBlock>
            </GanttRow>
        ...
        <div class="content" ref="contentGrid">
          ...
          <div class="gantt-grid" ref="box" @contextmenu.prevent="() => { }">
            <!--   网格状图表  -->
            <GanttRow v-for="(item, rowInd) in ganttData" :key="rowInd" :id="`ganttRow-${rowInd}`"
              class="gantt-row-relative" :class="{ 'gantt-row-active': nowSuck === rowInd }">
              <template>
                <div v-for="(times, index) in rowBlocks(item[listKey])" :key="`times-${rowInd}-${index}`"
                  :style="_timeBlockStyle(times)" class="time-block gantt-grab" :class="' block-type-' + times.status"
                  @mousedown="onMouseDown($event, times.id, rowInd, index)"
                  @contextmenu.prevent="onRightClick($event, item[listKey], times, `times${rowInd}${index}`)"
                  @click.stop="onClicked(times, index, `times${rowInd}${index}`)">
                  <span :class="['time-drag time-drag-left']" v-if="rowIndex == `times${rowInd}${index}`"></span>
                  {{ times.projectName }}
                  <span class="time-drag time-drag-right" v-if="rowIndex == `times${rowInd}${index}`"></span>
                </div>
              </template>
              <!--      block => 180  因为[this.listKey] => 60    60*3 = 180px   -->
              <GanttBlock class="time" v-for="rowIndex in choiceTimeArr.length" :key="rowIndex" style="flex-shrink: 0">
              </GanttBlock>
    ...
    <!-- 编辑窗口 -->
    <el-dialog title="编辑时间块" :visible.sync="isEditBlock">
      <el-form v-if="editBlock" :model="editBlock" label-width="120px">
        <el-form-item label="名称"><el-input v-model="editBlock.projectName" autocomplete="off"></el-input></el-form-item>
...
import GanttTimeBlock from './gantt-time-block';
import ganttMix from './gantt-mix.js';
import ganttHisMix from './gantt-his-mix.js';
...
export default {
  ...
  data() {
    return {
      ...
      transFormXValue: 0,
      // 飞机的数据，用于渲染左侧边
      ganttData: data, //airPlaneData.getData(),
      // 选择的时间区间
      choiceTime: [new Date('2022-04-14 00:00:00'), new Date('2022-04-24 00:00:00')],
      defaultTop: 10,
      nowSuck: -1,
      // X轴移动距离
      moveX: 0,
  ...
  methods: {
     ...
     * @param blockId 时间块的ID
     * @param rowIndex 该条线的id
     */
    onMouseDown(e, blockId, rowIndex, index) {
      // 删除模式下不处理拖动事件
      ...
      // 只移动time-block元素
      const dom = e.target // e.target.className.indexOf('time-block') > -1 ? e.target : e.target.parentElement;
      const timeDragDom = e.target;
      ...
      this.index = index;
      // 处理边界
      const MaxWidth = document.querySelector('.gantt-grid').offsetWidth - dom.offsetWidth;
      // 让本来拥有手掌样式的class取消
      ...
      box.onmousemove = (ee) => {
        ...
        this.nowSuck = suck;
        // 移动事件
        this.onMouseMove(ee, disX, disY, dom, timeDragDom, MaxWidth, e.clientX, dom.offsetWidth);
      };
      ...
      window.onmouseup = () => {
        ...
        const startTime = new Date(this.choiceTime[0].getTime() + (parseInt(dom.style.left) * 3600000) / 15);
        ...
        // 加入新数据
        const data = JSON.parse(JSON.stringify(oldTimeBlock));
        // 更新开始时间和结束时间
        ...
        // self.ganttData[suck][this.listKey][this.index] = data;
        // this.$set(self.ganttData[suck][this.listKey], index, data)
    ...
    // 时间块点击
    onClicked(times, index, rowIndex) {
      this.$nextTick(() => {
        this.rowIndex = rowIndex;
        console.log(' this.rowIndex------', this.rowIndex);
      });
      if (this.isDeleteMode) {
        this.onDeleteBlock(times);
      }
    },
     ...
     */
    onMouseMove(e, disX, disY, targetDom, timeDragDom, MaxWidth, clientX, offsetWidth) {
      e.stopPropagation();
      ...
      this.moveX = left;
      this.moveY = top;
      // const offsetWidth = e.target.parentElement.offsetWidth;
      // 移动位置不能小于零(开始时间)
      if (left < 0) left = 0;
      ...
      return;
      const widthNum = parseInt(targetDom.style.width.replace(/px/g, ''));
      // 处理左右拖动控制任务条的宽度
      if (timeDragDom.className.indexOf('time-drag') > -1) {
        // targetDom.style.top = '10px';
        if (timeDragDom.className.indexOf('time-drag-left') > -1) {
          // let w = e.clientX < clientX ? (targetDom.offsetWidth + left ) : (targetDom.offsetWidth - (e.clientX - clientX));
          let w = offsetWidth + (clientX - e.clientX);
          console.log('left------', e.clientX < clientX);
          console.log('left------', clientX - e.clientX, w > 0 ? w : 0, offsetWidth, widthNum);
          console.log(' w > 0 ? w : 0------', (w > 0 ? w : 0) + 'px');
          if (w > -1) {
            targetDom.style.width = w + 'px';
            targetDom.style.left = left + 'px';
          }

        }
        if (timeDragDom.className.indexOf('time-drag-right') > -1) {
          // timeDragDom.style.right = '0px';
          // targetDom.style.width = left < e.clientX ? targetDom.offsetWidth - left + 'px' : targetDom.offsetWidth + left + 'px';
        }
    ...
    onRightClick(event, row, block, rowIndex) {
      //编辑需要把时间长度先计算好
