		...
		_timeBlockStyle() {
			return (block) => {
				...
				if (block && block.startTime && block.endTime) {
					// 一个小时长度为15px
					left = (this._getTime(block.startTime) - this._getTime(this.choiceTime[0])) / 3600000 * 15
					width = (this._getTime(block.endTime) - this._getTime(block.startTime)) / 3600000 * 15
				}
				const style = {
